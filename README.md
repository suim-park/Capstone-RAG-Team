...
This project aims to **build a medical question answering chatbot leveraging GraphRAG** (Graph Retrieval-Augmented Generation) technology. The idea behind RapgRAG is to leverage structural reasoning of knowledge graph and combine with the abilities of large language model in contextual generation. 
Research papers from Wiki Journal Club—a free, open-source dataset that includes papers from reputable medical journals, were used to construct knowledge graph. Leveraging GraphRAG, our chatbot summarizes key findings across multiple research papers based on the generated knowledge graph, synthesizing information to provide concise and reliable insights in response to specific queries.  This capability demonstrates how the chatbot can significantly **reduce the time** physicians need to access critical information for clinical decision-making.
