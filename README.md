...
Research papers from Wiki Journal Club—a free, open-source dataset that includes papers from reputable medical journals, were used to construct knowledge graph. Leveraging GraphRAG, our chatbot constructs a high-level knowledge graph that summarizes key findings across multiple research papers, synthesizing information to provide concise and reliable insights in response to specific queries.  This capability demonstrates how the chatbot can significantly **reduce the time** physicians need to access critical information for clinical decision-making.
